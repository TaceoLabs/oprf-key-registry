...
interface DeploymentConfig {
  ...
  accumulator_address: Hex
  key_gen_verifier_address: Hex
  threshold: number
  numPeers: number
  salt: Hex
  network: NetworkName
...
async function buildDeploymentTransactions(config: DeploymentConfig): Promise<{
  ...
  const { threshold, numPeers, salt, safeAddress, key_gen_verifier_address, accumulator_address } = config
  ...
  const implSalt = keccak256(concat([salt, '0x07']))
  ...
  const initData = encodeFunctionData({
    ...
    args: [
      ...
      key_gen_verifier_address,       // keyGenVerifier
      accumulator_address,    // accumulator
      BigInt(threshold),
      BigInt(numPeers),
    ],
  })
  ...
  return {
    ...
    addresses: {
      ...
      verifier: key_gen_verifier_address,
...
async function deployLocal(config: DeploymentConfig) {
  ...
  console.log('📍 Provided addresses:')
...
async function main() {
  ...
  const config: DeploymentConfig = {
    ...
    accumulator_address: process.env.ACCUMULATOR_ADDRESS as Hex,
    key_gen_verifier_address: process.env.KEY_GEN_VERIFIER_ADDRESS as Hex,
  ...
  if (!config.accumulator_address) {
    console.error('ACCUMULATOR_ADDRESS environment variable required')
    process.exit(1)
  }
  if (!config.key_gen_verifier_address) {
    console.error('KEY_GEN_VERIFIER_ADDRESS environment variable required')
    process.exit(1)
  }
  ...
  console.log('  accumulator address:', config.accumulator_address)
  console.log('  key-gen verifier address:', config.key_gen_verifier_address)
